...
Error FrameBuffer::Initialize(const FrameBufferConfig &config)
{
    ...
    const auto bpp = BitsPerPixel(config_.pixel_format);
    if (bpp <= 0)
    {
    ...
    {
        buffer_.resize(
            ((bpp + 7) / 8) * config_.horizontal_resolution * config_.vertical_resolution);
...
Error FrameBuffer::Copy(Vector2D<int> pos, const FrameBuffer &src)
{
    ...
    const auto bytespp = (bpp + 7) / 8;
    const auto bytes_per_copy_line = bytespp * (copy_end_dst_x - copy_start_dst_x);

    uint8_t *dst_buf = config_.frame_buffer + bytespp * (config_.pixels_per_scan_line * copy_start_dst_y + copy_start_dst_x);
    ...
    for (int dy = 0; dy < copy_end_dst_y - copy_start_dst_y; ++dy)
    {
        ...
        dst_buf += bytespp * config_.pixels_per_scan_line;
        src_buf += bytespp * src.config_.pixels_per_scan_line;
    }
