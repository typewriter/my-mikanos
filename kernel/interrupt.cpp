...
#define FaultHandlerWithError(fault_name)                                                              \
    ...
    {                                                                                                  \
        ...
        WriteString(*pixel_writer, {500, 16 * 4}, "ERR", {0, 0, 0});                                   \
...
void PrintHex(uint64_t value, int width, Vector2D<int> pos)
{
    for (int i = 0; i < width; ++i)
    {
        ...
        WriteAscii(*pixel_writer, pos + Vector2D<int>{8 * i, 0}, x, { 0, 0, 0 });
    }
}

void PrintFrame(InterruptFrame* frame, const char* exp_name)
{
    WriteString(*pixel_writer, { 500, 16 * 0 }, exp_name, { 0, 0, 0 });
    WriteString(*pixel_writer, { 500, 16 * 1 }, "CS:RIP", { 0, 0, 0 });
    PrintHex(frame->cs, 4, { 500 + 8 * 7, 16 * 1 });
    PrintHex(frame->rip, 16, { 500 + 8 * 12, 16 * 1 });
    WriteString(*pixel_writer, { 500, 16 * 2 }, "RFLAGS", { 0, 0, 0 });
    PrintHex(frame->rflags, 16, { 500 + 8 * 7, 16 * 2 });
    WriteString(*pixel_writer, { 500, 16 * 3 }, "SS:RSP", { 0, 0, 0 });
